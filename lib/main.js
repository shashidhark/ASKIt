...
if (typeof ss.storage.version === "undefined" || ss.storage.version != 0.2) {
    //ss.storage.firstRun = false;
    ss.storage.version = 0.2;
        	...
        	//console.log(response.status);
        	    if(response.status==0 || response.status==504 || response.status==408){
                    ...
                    worker.port.emit("error", "");
		          	...
		          	worker.port.emit("completeRes", [response.text, ss.storage.prefs_data.searchLink, searchLink[ss.storage.prefs_data.searchLink]]);
		        }
            }
      	  }).get();
      	}
