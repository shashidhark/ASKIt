...
var version="0.3.6"; //Change here if new version
...
var panel = panels.Panel({
	...
	contentScriptFile	: [data.url("js/math.min.js"), data.url("js/jquery-1.6.min.js"), data.url("panel.js")]
});
	...
	panel.port.on("closeBubble", function(){
		...
		if(selectedText){
			...
			var window = getTabContentWindow(getTabForId(tab.id));
  			panel.port.emit("takeSelectionFromTab", window.getSelection().toString());//selectedText);
  		}
  		else{
